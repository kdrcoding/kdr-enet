...
2. On the car laptop, click **Radmin VPN**. The address, such as `tcp://26.x.x.x:6801`, sits above **Copy**. **Copy** is next to **Start**.
3. Click **Start**. The window stays **Not connected** until the other laptop reaches this one. Then **Connected from** that laptop's Radmin address turns green. If they leave, it goes back to **Not connected**. The address stays on the one line above **Copy**.
